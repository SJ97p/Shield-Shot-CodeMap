...
코드 구조만 보고 V2가 빠를 것이라고 결론 내리지 않았습니다. 활과 방패를 동시에 다루는 입력이 실제로 더 매끄럽게 느껴졌지만, 사람의 손으로 직접 드래그하면 경로·속도·샘플 수와 프레임이 매번 달라져 두 버전의 차이를 설명할 수 없습니다. 그래서 동일한 입력량과 실행 조건을 보장하는 결정론적 벤치마크를 별도로 구축했습니다.
